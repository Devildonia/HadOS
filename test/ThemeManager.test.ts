...
describe('ThemeManager', () => {
    ...
    beforeEach(() => {
        ...
        (window as any).ShaderWallpaper = {
            setFragmentShader: vi.fn()
        };
        Services.register('ShaderWallpaper', (window as any).ShaderWallpaper);
    ...
    describe('applyTheme', () => {
        ...
        it('should correctly mutate DOM text content for specific IDs', () => {
            ...
            expect(welcomeText.textContent).toBe('Welcome to HadOS UI v1.0.2!');

            // Win95 theme text
            themeManager.applyTheme('win95');
            expect(startMenuTitle.textContent).toBe('HadOS');
            expect(welcomeText.textContent).toBe('Welcome to HadOS v1.0.2!');
        });
    });
});
