 ...
 */
let seq = 0;
function makeFakeApp(appId: string, geometry = { left: 120, top: 80, width: 400, height: 300 }) {
    class FakeApp {
        windowId: string;
        constructor() {
...
describe('SessionManager (Fase 5)', () => {
    ...
    it('skips worker/iframe child processes (only windowed apps are recorded)', () => {
        ...
        Kernel.spawnWorker('some-worker', { postMessage() {}, onMessage() {}, terminate() {} } as any);
    ...
    it('persists the session to the VFS and reads it back', () => {
        ...
        const read = SessionManager.read() as any;
