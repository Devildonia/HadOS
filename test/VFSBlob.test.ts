...
describe('VFS binary/blob API (Phase 0.2)', () => {
    ...
    it('writes a Blob out-of-tree and reads it back', async () => {
        ...
        const node = VFS.resolve('C:\\DOCUMENTS\\pic.png') as any;
        ...
        expect((out as Blob).type).toBe('image/png'); // MIME preserved on read-back
        expect(await (out as Blob).text()).toBe('PNG-BYTES-HERE');
    });
    ...
    it('releases the blob when the file is deleted', async () => {
        ...
        const ref = (VFS.resolve('C:\\DOCUMENTS\\gone.png') as any).blobRef;
    ...
    it('frees the old blob when overwriting with inline text', async () => {
        ...
        const oldRef = (VFS.resolve('C:\\DOCUMENTS\\swap') as any).blobRef;
    ...
    it('rejects a blob over the 50 MB cap', async () => {
        ...
        const ok = await VFS.writeFileAsync('C:\\DOCUMENTS', 'huge.bin', huge as any);
