...
import { PackageManager } from '../js/core/PackageManager';
import { validatePackage } from '../js/core/AppPackage';
import { attachSyscalls } from '../js/core/SyscallBroker';
import { WorkerProcess, messagePortTransport } from '../js/core/WorkerProcess';
import { AppRuntime, messagePortGuestTransport } from '../js/sdk/appRuntime';
...
describe('Audit v1.6.6 — syscall hardening (M2, B2)', () => {
    ...
    it('B2: notify falls back to info for a non-allow-listed level', async () => {
        ...
        const { Services } = await import('../js/core/ServiceContainer');
        Services.unregister('Notify');
        Services.register('Notify', fakeNotify as any);
...
describe('Audit v1.6.6 — PermissionBroker (M1, B1)', () => {
    ...
    it('B1: the consent dialog escapes the app id (no HTML injection)', async () => {
        ...
        (document.querySelector('[data-consent="denied"]') as HTMLButtonElement).click();
        await p;
    });
});
...
describe('Audit v1.6.6 — packaging (M2, O1)', () => {
    ...
    it('M2: rejects a manifest whose entry escapes the package', () => {
        ...
        expect(validatePackage(bad as any).error).toMatch(/safe relative path/);
    });

    it('O1: verify() detects a tampered package against the recorded integrity', async () => {
        await PackageManager.install(pkg());
        expect(await PackageManager.verify('demo-app', pkg() as any)).toBe(true);
        expect(await PackageManager.verify('demo-app', pkg({ 'index.html': '<h1>tampered</h1>' }) as any)).toBe(false);
        expect(await PackageManager.verify('not-installed', pkg() as any)).toBe(false);
    });
});
...
describe('Audit v1.6.6 — VFS blob write race (B4)', () => {
    ...
    it('B4: concurrent blob writes to the same path leave a readable winner', async () => {
        ...
        expect(['AAA', 'BBB']).toContain(await (out as Blob).text());
    });
});
