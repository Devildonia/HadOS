...
describe('UI Modules', () => {
    ...
    describe('MessageLibrary', () => {
        ...
        it('should return a random message from a specific category', () => {
            ...
            const rawKeys = (msgLib as any).messages['GREETINGS'];
            const i18nService = Services.get('i18n') as any;
            const expected = i18nService ? rawKeys.map((k: string) => i18nService.t(k)) : rawKeys;
            expect(expected).toContain(msg);
        });

        it('should avoid repeating recent messages', () => {
            (msgLib as any).maxHistory = 2;
            (msgLib as any).messages['TEST_CAT'] = ['A', 'B']; // Only 2 messages
    ...
    describe('BubbleAnimator', () => {
        ...
        it('should play a fade in animation to completion over duration', () => {
            ...
            expect((animator as any).animations.has('test_fade')).toBe(false);
        });
        ...
        it('should cancel active animations', () => {
            ...
            expect(onUpdate).toHaveBeenCalled();
            expect((animator as any).animations.has('test_wobble')).toBe(true);
            ...
            expect((animator as any).animations.has('test_wobble')).toBe(false);
        });
