...
import { validateManifest, compareVersions, packageIntegrity } from '../js/core/AppPackage';
...
describe('PackageManager install/update/uninstall (Fase 4)', () => {
    ...
    it('installs a package into the app home and records it', async () => {
        ...
        const installed = PackageManager.get('demo-app') as any;
    ...
    it('rejects a package whose entry file is missing', async () => {
        const res = await PackageManager.install({ manifest: pkg().manifest, files: { 'other.txt': 'x' } } as any);
        expect(res.ok).toBe(false);
    ...
    it('updates to a newer version and removes stale files', async () => {
        ...
        expect((PackageManager.get('demo-app') as any).manifest.version).toBe('1.1.0');
    ...
    it('persists the registry and restores permission ceilings on init', async () => {
        ...
        expect((PackageManager.get('demo-app') as any).manifest.name).toBe('Demo App');
