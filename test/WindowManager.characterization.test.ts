...
beforeEach(() => {
    ...
    Services.register('AudioManager', { play: vi.fn(), loadSound: vi.fn(), init: vi.fn() } as any);
...
describe('WindowManager (caracterización) — iframes', () => {
    it('al cerrar guarda el src en data-src y descarga a about:blank', () => {
        const iframe = document.getElementById('the-frame') as HTMLIFrameElement;
...
describe('WindowManager (caracterización) — sonidos por tema', () => {
    it('con tema modern reproduce open_window_modern al abrir', () => {
        Services.register('ThemeManager', { currentTheme: 'modern' } as any);
        const audio = Services.get('AudioManager') as any;
    ...
    it('sin tema modern NO reproduce sonido de ventana', () => {
        Services.register('ThemeManager', { currentTheme: 'classic' } as any);
        const audio = Services.get('AudioManager') as any;
